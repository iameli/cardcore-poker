<script>
  ...
  const visibleCommunity = $derived(
    gamePhase === 'preflop' ? [] :
    gamePhase === 'flop' ? communityCards.slice(0, 3) :
    gamePhase === 'turn' ? communityCards.slice(0, 4) :
    gamePhase === 'river' || gamePhase === 'showdown' ? communityCards :
    []
  );
  ...
  const seatMap = $derived(
    Object.fromEntries(
      Object.entries(players).map(([id, p]) => [p.seat, { id, player: p }])
    )
  );
...
<style>
  ...
  .seat-area.top { grid-row: 1; }
  .seat-area.top:nth-child(1) { grid-column: 1; }
  .seat-area.top:nth-child(2) { grid-column: 2; }
  .seat-area.top:nth-child(3) { grid-column: 3; }

  .seat-area.left { grid-column: 1; grid-row: 2; }

  .seat-area.right { grid-column: 3; grid-row: 2; }

  .seat-area.bottom { grid-row: 3; }
  .seat-area.bottom:nth-child(7) { grid-column: 1; }
  .seat-area.bottom:nth-child(8) { grid-column: 2; }
  .seat-area.bottom:nth-child(9) { grid-column: 3; }
