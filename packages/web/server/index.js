    ...
    switch (msg.type) {
      case "join": {
        ...
        if (existing) {
          ...
          console.log(`[JOIN] ${playerName} (${playerId}) reconnected to room ${roomId}`);
          ...
          if (room.gameActive) {
            ...
            ws.isPending = true;
            console.log(
              "[JOIN] " + playerName + " (" + playerId + ") queued for next hand in room " + roomId,
            );
            ws.send(
              JSON.stringify({
                type: "game_state_sync",
      ...
      case "sit": {
        ...
        console.log(
          `[SIT] ${playerName} (${playerId}) moving from seat ${player.seat} to seat ${msg.seat} in room ${roomId}`,
        );
      ...
      case "ready": {
        ...
        if (!player) return;
        player.ready = true;
        console.log(`[READY] ${playerName} (${playerId}) is ready in room ${roomId}`);
        ...
        if (room.players.length >= 2 && room.players.every((p) => p.ready)) {
          console.log(`[GAME] All players ready in room ${roomId}, starting game!`);
      ...
      case "action": {
        ...
        if (
          room &&
          msg.action &&
          (msg.action.type === "wasm_table" || msg.action.type === "wasm_action")
        ) {
      ...
      case "spectate": {
        ...
        if (room.gameActive && room.gamePlayers) {
          console.log(`[SPECTATE] Game in progress, sending game_state_sync to ${playerName}`);
          ws.send(
            JSON.stringify({
              type: "game_state_sync",
              players: room.gamePlayers,
      ...
      case "leave": {
        console.log(`[LEAVE] ${playerName} (${playerId}) leaving room ${roomId}`);
        leaveRoom(ws);
        break;
      }
    }
...
function leaveRoom(ws) {
  ...
  if (ws.isSpectator) {
    ...
    console.log(
      `[LEAVE] Spectator ${leavingSpectator?.name || "Unknown"} (${pid}) left room ${rid}`,
    );
  ...
  console.log(
    "[LEAVE] " + (leavingPlayer?.name || "Unknown") + " (" + pid + ") removed from room " + rid,
  );
  if (room.gameActive) {
    broadcastRoom(
      rid,
      {
        ...
        action: {
          type: "wasm_action",
          cbor: "omUkdHlwZXgYcmUuY2FyZGNvLnBva2VyLmRlZnMjYmV0ZmFjdGlvbmRmb2xk",
        },
      },
      pid,
    );
